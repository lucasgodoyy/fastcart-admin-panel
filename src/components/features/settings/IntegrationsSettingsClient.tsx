...
export function IntegrationsSettingsClient() {
  ...
  const openStripeOnboardingMutation = useMutation({
    ...
    onError: () => toast.error('Falha ao iniciar conexao com Stripe.'),
  });
  ...
  const openMelhorEnvioAuthorizeMutation = useMutation({
    ...
    onError: () => toast.error('Falha ao iniciar conexao com Melhor Envio.'),
  });
  ...
  const connectMelhorEnvioMutation = useMutation({
    ...
    onError: () => {
      toast.error('Falha ao finalizar conexao com Melhor Envio.');
      router.replace('/admin/settings/integrations');
    },
  });
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      toast.error('Autorizacao do Melhor Envio cancelada ou invalida.');
      router.replace('/admin/settings/integrations');
      return;
    }
    ...
    if (oauthState && !oauthState.startsWith('store-')) {
      toast.error('State invalido no retorno do Melhor Envio.');
      router.replace('/admin/settings/integrations');
      return;
    }
    ...
    >
      <div className="grid gap-4">
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          {stripeStatus?.connected && (
            <div className="mb-4 space-y-1 text-xs text-muted-foreground">
              ...
              <p>Charges enabled: {stripeStatus.chargesEnabled ? 'Sim' : 'Nao'}</p>
              <p>Payouts enabled: {stripeStatus.payoutsEnabled ? 'Sim' : 'Nao'}</p>
            </div>
          )}
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          {melhorEnvioStatus?.connected && (
            <div className="mb-4 space-y-1 text-xs text-muted-foreground">
              <p>Conta do provedor: {melhorEnvioStatus.providerAccountId || 'Nao informado'}</p>
              <p>
                Conectado em:{' '}
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            O lojista pode criar a conta no Melhor Envio durante o fluxo de autorizacao.
          </p>
        </div>
      </div>
      ...
      <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        Modelo multi-tenant: cada loja conecta sua propria conta Stripe e Melhor Envio.
      </div>
    </SettingsPageLayout>
  );
}
